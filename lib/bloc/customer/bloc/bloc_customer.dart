  ...
  BlocCustomer({required this.context}) : super(const BlocCustomerInitial()) {
    on<BlocCustomerLoadListEvent>((event, emit) async {
      ...
      if (data != null) {
        ...
        emit(BlocCustomerLoadListSuccess(listCustomer: res, page: event.page));
      }
    ...
    on<BlocCustomerLoadCoinEvent>((event, emit) async {
      ...
      if (data != null) {
        emit(BlocCustomerLoadCoinSuccess(coin: int.parse(data['so_du'])));
      }
      emit(const BlocCustomerInitial());
    });
    ...
    on<BlocCustomerDetailEvent>((event, emit) async {
      ...
      if (data != null) {
        emit(BlocCustomerLoadDetailsSuccess(customer: event.customerInfo, customerData: data));
      }
      emit(const BlocCustomerInitial());
    });
