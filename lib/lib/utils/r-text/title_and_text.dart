...
class RTitleAndText extends StatelessWidget {
  ...
  const RTitleAndText({
    ...
    this.textOverflow,
    this.contentWidth,
    this.titleType = RTextType.subtitle,
    this.textType = RTextType.text,
  }) : super(key: key);
  @override
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          RText(title: title, type: titleType, color: themeColor),
          if (maxLines == null || textOverflow == null)
            RText(title: text ?? '', type: textType)
          else
            Container(
                alignment: Alignment.centerRight,
                width: contentWidth,
                child: RText(title: text ?? '', type: textType, maxLines: maxLines, textOverflow: textOverflow)),
        ],
      ),
    );
  }
